...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('recharge_item')
    .setDescription('Recharge an item to its max uses')
    .addStringOption(option =>
      option
        .setName('name')
        .setDescription('Item to recharge')
        .setRequired(true)
  ...
  async execute(interaction) {
    ...
    if (!item) {
      await replySafely(interaction, { content: `Your active specialist does not have an item named ${name}.`, ephemeral: true });
      return;
    }
